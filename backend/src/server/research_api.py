...
async def get_research_projects():
    ...
    try:
        ...
        return [ResearchProjectResponse(id=p.id, title=p.title, description=p.description, status=p.status, tags=p.tags, created_at=p.created_at.isoformat(), updated_at=p.updated_at.isoformat()) for p in projects]
    except Exception as e:
...
async def get_research_project(project_id: int):
    ...
    try:
        ...
        return ResearchProjectResponse(id=project.id, title=project.title, description=project.description, status=project.status, tags=project.tags, created_at=project.created_at.isoformat(), updated_at=project.updated_at.isoformat())
    ...
    try:
        ...
        return [
            ResearchSessionResponse(
                id=s.id, project_id=s.project_id, session_id=s.session_id, title=s.title, status=s.status, created_at=s.created_at.isoformat(), updated_at=s.updated_at.isoformat(), message_count=len(research_db.get_session_messages(s.id))
            )
            for s in sessions
        ]
    except Exception as e:
...
async def get_session_messages(session_id: int):
    ...
    try:
        ...
        return [SessionMessageResponse(id=m.id, session_id=m.session_id, role=m.role, content=m.content, message_type=m.message_type, tool_calls=m.tool_calls, created_at=m.created_at.isoformat()) for m in messages]
    except Exception as e:
...
async def get_project_documents(project_id: int):
    ...
    try:
        ...
        return [ResearchDocumentResponse(id=d.id, project_id=d.project_id, title=d.title, content=d.content, source_url=d.source_url, document_type=d.document_type, created_at=d.created_at.isoformat()) for d in documents]
    except Exception as e:
...
async def create_research_project(title: str, description: str = "", tags: str = ""):
    ...
    try:
        ...
        return {"id": project.id, "title": project.title, "description": project.description, "status": project.status, "tags": project.tags, "created_at": project.created_at.isoformat()}
    except Exception as e:
...
async def create_research_finding(project_id: int, title: str, content: str, category: str = "insight", confidence: float = 0.0, source_documents: str = "", tags: str = ""):
    ...
    try:
        ...
        return {"id": finding.id, "title": finding.title, "content": finding.content, "category": finding.category, "confidence": finding.confidence, "created_at": finding.created_at.isoformat()}
    except Exception as e:
...
async def create_research_document(project_id: int, title: str, content: str = "", source_url: str = "", document_type: str = "web"):
    ...
    try:
        ...
        return {"id": document.id, "title": document.title, "content": document.content, "source_url": document.source_url, "document_type": document.document_type, "created_at": document.created_at.isoformat()}
    except Exception as e:
