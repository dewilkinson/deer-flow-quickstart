...
from .shared_storage import GLOBAL_CONTEXT
import json
...
BLACKLISTED_COMMANDS = [
    "format", "shred", "dd", "mkfs", "parted", "fdisk", "chown", "chmod", "sudo", "su"
]
...
@tool
def bash_shell_tool(
    command: Annotated[str, "The bash/shell command to execute safely."]
):
    ...
    try:
        ...
        if any(sens in base_cmd for sens in SENSITIVE_COMMANDS):
            return json.dumps({
                ...
                "command": command,
                "reason": f"Sensitive operation detected: '{base_cmd}'. Please confirm you wish to proceed."
            })
