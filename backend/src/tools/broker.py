...
def get_brokerage_history(account_id: str, days: int = 30, config: RunnableConfig = None):
    ...
    try:
        ...
        api_response = client.transactions_and_reporting.get_activities(user_id=user_id, user_secret=user_secret, accounts=account_id, start_date=start_date.strftime("%Y-%m-%d"), end_date=end_date.strftime("%Y-%m-%d"))
        return api_response
    except Exception as e:
        logger.error(f"SnapTrade API Error: {e}")
