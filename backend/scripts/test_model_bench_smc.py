...
async def benchmark_model(model_name: str, symbols: list[str]) -> list[dict]:
    ...
    llm_with_tools = llm.bind_tools([get_smc_analysis])
    
    results = []
    
    for sym in symbols:
        nonce = uuid.uuid4().hex[:6]
        prompt = f"Run an SMC analysis on {sym}. [nonce: {nonce}]"
        ...
        try:
            ...
            if hasattr(res1, "tool_calls") and res1.tool_calls:
                ...
                tool_result = await get_smc_analysis.ainvoke(tc["args"])
                ...
                res2 = await asyncio.wait_for(llm.ainvoke(msgs), timeout=25.0)
...
async def main():
    ...
    print("\n" + "="*len(header))
    print("END-TO-END SMC BENCHMARK RESULTS".center(len(header)))
    print("="*len(header))
    print(header)
    print("-" * len(header))
