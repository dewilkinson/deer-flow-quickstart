...
async def test_portfolio_manager_node_execution():
    ...
    with patch("src.graph.nodes.portfolio_manager._setup_and_execute_agent_step", new_callable=AsyncMock) as mock_exec:
        ...
        state = State(current_plan=Plan(title="Portfolio Oversight", thought="", steps=[], direct_response="", locale="en-US", has_enough_context=False), research_topic="Strategic Review", verbosity=1)
