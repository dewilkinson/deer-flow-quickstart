...
async def test_terminal_specialist_interrupt():
    ...
    state = {"messages": [HumanMessage(content="Terminal, delete the test file.", name="user")], "is_test_mode": True, "steps_completed": 0, "current_plan": "test plan"}
    ...
    with patch("src.graph.nodes.terminal_specialist._setup_and_execute_agent_step") as mock_exec:
        mock_exec.return_value = {"messages": [AIMessage(content='{"status": "APPROVAL_REQUIRED", "command": "rm test.txt", "reason": "Sensitive operation"}', name="terminal_specialist")]}
...
async def test_vision_specialist_purge():
    ...
    multimodal_content = [{"type": "text", "text": "Analyze this chart."}, {"type": "image_url", "image_url": "data:image/png;base64,IMAGEDATA..."}]
    state = {"messages": [HumanMessage(content=multimodal_content, name="user")], "is_test_mode": True, "steps_completed": 0, "current_plan": "test plan"}
    config = {"configurable": {"thread_id": "test_thread"}}

    with patch("src.graph.nodes.vision_specialist._setup_and_execute_agent_step") as mock_exec:
        mock_exec.return_value = {"messages": [AIMessage(content="Analysis: Bullish", name="vision_specialist")]}
